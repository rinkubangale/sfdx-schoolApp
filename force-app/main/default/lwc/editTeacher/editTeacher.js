...
export default class EditTeacher extends LightningElement {
    ...
    handleSelectValue(event) {
        // this.DepartmentName = event.detail && JSON.parse(JSON.stringify(event.detail));
        // console.log("I'm Department Name ", JSON.stringify(event.detail.Id));
        let ids = event.detail && JSON.parse(JSON.stringify(event.detail));
        console.log(ids);
        // ids.map((id)=>{
        //     this.prevDepartmentIds.push(id);
        // });
        this.allDeptIds = ids;
        // console.log(JSON.stringify(this.getUnique(this.prevDepartmentIds)));
    }

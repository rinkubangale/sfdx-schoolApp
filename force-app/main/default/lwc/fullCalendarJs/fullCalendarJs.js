...
export default class FullCalendarJs extends NavigationMixin(LightningElement) {
    ...
    eventObj(value){
        ...
        if(data){
            ...
            console.log(data);
            let events = data.map(exam => {
                return { id : exam.recordId, 
                        title : exam.Name, 
                        start : exam.Start_Date__c,
                        end : exam.End_Date__c,
                        description : exam.Class__r.Name,
                        //allDay : exam.IsAllDayEvent
                      };
            });
   ...
   renderedCallback() {
    ...
    handleSelectValue(event) {
        this.class = JSON.stringify(event.detail);
    }
   //To save the event
    handleSave(event) {
        ...
        let newevent = {title : this.title, start : this.startDate, end: this.endDate, description : this.description, class__c : "5th" };

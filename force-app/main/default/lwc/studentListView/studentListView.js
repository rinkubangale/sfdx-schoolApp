...
export default class StudentListView extends LightningElement {
  fldsItemValues = [];
  total = 0;
  active;
  allList;
  ...
  handleRowAction(event) {
    ...
    if (event.detail.action.label === "Edit") {
      ...
      this.isModalOpen = true;
      this.className = this.dataList.find(
        (x) => x.Id == event.detail.row.Id
      ).Class_Name__c;
      this.classSection = this.dataList.find(
        (x) => x.Id == event.detail.row.Id
      ).Class_Section__c;
      this.Year = this.dataList.find(
        (x) => x.Id == event.detail.row.Id
      ).Academic_Year__c;
      console.log(JSON.stringify(this.Year));
      console.log(
        JSON.stringify(this.dataList.find((x) => x.Id == event.detail.row.Id))
      );
    }
  }

  getSelectedStudent() {
    var selectedStuds = this.template

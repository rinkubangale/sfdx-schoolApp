...
export default class AdmissionListView extends LightningElement {
    @track options = [];
    @track sortBy;
    @track sortDirection;
    @track dataListcopy = [];
    ...
    wiredFetchAY({ error, data }) {
        if (data) {
            ...
            for (let x of data) {
                obj.push({
                    'label': x.From_Year__c + '/' + this.academicYear(x.From_Month__c) + ' - ' + x.To_Year__c + '/' + this.academicYear(x.To_Month__c),
    ...
    getsearchRecords(event) {
        ...
        this.searchData = this.searchData.split('/').join('-');
        console.log('Dtaaa', this.searchData)
    }
